    ...
    return (
        <div>
            <Title align='left' font='outfit' title='Dashboard'
                subTitle='Monitor your room listings, track bookings and analyze revenue-all in one place. stay updated with
             real-time insights to ensure smooth operations' />

            <div className='flex gap-4 my-8'>
                {/* { Total Bookings} */}
                <div className='bg-primary/30 border border-primary/10 rounded flex p-4 *:pr-8'>
                    <img src={assets.totalBookingIcon} alt="" className='max-sm:hidden h-10' />
                    <div className='flex flex-col sm:ml-4 font font-medium'>
                ...
                {dashboardData.bookings.map((items, index)=>(
                    <tr key={index}>
                        ...
                        <td className='py-3 px-4 text-gray-700 border-t border-gray-300 flex'>
                            <button className={`py-1 px-3 text-xs rounded-full mx-auto ${items.isPaid? 'bg-green-200 text-green-600' : 'bg-amber-200 text-yellow-600'} `}>
